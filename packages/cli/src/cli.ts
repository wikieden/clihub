...
// ─── wizard ───────────────────────────────────────────────────────────
cli
  .command('wizard', 'Guided first-run setup: install CLIs, preset, proxy, accounts, config')
  ...
  .action(async (opts: { dryRun?: boolean }) => {
    ...
    p.intro('clihub wizard — first-run setup');
    ...
    // 4. accounts (multi-profile + one key each; multi-key loop is a follow-up)
    ...
    for (const id of toolIds) {
      ...
      if (!pr) continue;
      try { await pr.install({}); ok(`installed ${id}`); } catch (e) { warn(`install ${id}: ${e instanceof Error ? e.message : String(e)}`); }
    }
    if (proxy) {
    ...
    for (const a of accounts) {
      ...
      if (a.apiKeyName && a.apiKeyValue) {
        try { await setSecret(a.profile, a.apiKeyName, a.apiKeyValue); ok(`stored ${a.apiKeyName} for ${a.profile}`); }
        catch (e) { warn(`key ${a.profile}: ${e instanceof Error ? e.message : String(e)}`); }
      }
      ok(`profile ${a.profile}`);
    }
    if (accounts.length > 0) { try { await useProfile(accounts[0]!.profile, {}); ok(`active profile → ${accounts[0]!.profile}`); } catch { /* ignore */ } }
    ...
    ok('wrote clihub.yaml');
    if (schema) { await fsp.writeFile(path.join(cwd, 'clihub.schema.json'), clihubYamlSchemaJson(), 'utf8'); ok('wrote clihub.schema.json'); }
    if (memory) {
      const memPath = path.join(cwd, 'clihub.memory.md');
      const memExists = await fsp.access(memPath).then(() => true).catch(() => false);
      if (!memExists) { await fsp.writeFile(memPath, memoryTemplate(), 'utf8'); ok('wrote clihub.memory.md'); }
    }
