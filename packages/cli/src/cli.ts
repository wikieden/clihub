...
cli
  ...
  .action(async (action: string, opts: { port?: string; json?: boolean }) => {
    ...
    if (action === 'status') {
      ...
      const probe = await ctl.probeDaemon(state);
    ...
    if (action === 'stop') {
      ...
      if (res.kind === 'not-running') info('daemon not running');
      else if (res.kind === 'stopped') ok(`stopped daemon (pid ${res.pid})`);
      else warn(`pid ${res.pid} already gone — cleaned up state file`);
      if (res.kind !== 'not-running') {
    ...
    let res;
    try {
      res = await ctl.startDaemon({ port: opts.port });
    } catch (e) {
      err(e instanceof Error ? e.message : String(e));
      process.exit(1);
    }
    if (res.kind === 'already-running') {
      info(`daemon already running at ${res.state.url} (pid ${res.state.pid})`);
      return;
    }
    if (opts.json) { console.log(JSON.stringify({ url: res.state.url, port: res.state.port, pid: res.state.pid })); }
    else {
      ok(`daemon started at ${res.state.url}  (pid ${res.state.pid})`);
      info('bearer token written to ~/.clihub/daemon.json (0600) — clients read it from there.');
    }
    await (await import('@clihub/core')).appendAudit({ actor: 'cli', action: 'daemon.start', port: res.state.port }).catch(() => {});
  });
