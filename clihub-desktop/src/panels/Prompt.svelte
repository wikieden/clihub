<script lang="ts">
  ...
  async function load() {
    error = null;
    try {
      const res = await client.get<SyncPlanResponse>('/v1/prompt');
      file = res.file;
      plan = res.plan;
    } catch (e: unknown) {
  ...
  async function apply() {
    ...
    error = null;
    try {
      result = await client.post<SyncGenerateResult>('/v1/prompt/generate', {});
      await load();
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : String(e);
    } finally {
...
<section>
  <h1>Prompt</h1>
  <p class="hint">
    Cross-CLI system-prompt / persona sync — mirrors <code>clihub prompt generate</code>. Writes a
    second, independent managed block (own markers, own source <code>clihub.systemprompt.md</code>)
    into each CLI's instruction file alongside <code>Memory</code>'s block, so shared team rules and
    a distinct persona can coexist and be regenerated separately.
  </p>

  {#if error}<p class="error">{error}</p>{/if}

  {#if !file}
    <p class="muted">No system-prompt source found (clihub.systemprompt.md).</p>
  {:else}
